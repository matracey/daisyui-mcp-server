...
async function fetchComponentSlugs(): Promise<Set<string>> {
  log('Fetching component slugs from daisyui.com sitemap...')
  const response = await fetch('https://daisyui.com/sitemap.xml')
  if (!response.ok) {
    throw new Error(`Failed to fetch sitemap: HTTP ${response.status}`)
  }
  const xml = await response.text()
  const slugs = new Set<string>()
  const regex = /https:\/\/daisyui\.com\/components\/([^/]+)\//g
  let match: RegExpExecArray | null
  while ((match = regex.exec(xml)) !== null) {
    slugs.add(match[1])
  }
  log(`Found ${slugs.size} component slugs from sitemap`)
  return slugs
}

function findSlug(name: string, validSlugs: Set<string>): string | null {
  // Generate candidate slugs from the component name
  const kebab = name.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()
  const candidates = [
    kebab, // e.g. "chat-bubble", "browser-mockup"
    kebab.replace(/-/g, ''), // e.g. "chatbubble"
    kebab.split('-').reverse().join('-'), // e.g. "mockup-browser"
    kebab.split('-')[0], // e.g. "chat"
    kebab.replace(/s$/, ''), // e.g. "stat" (from "stats")
  ]
...
async function main() {
  ...
  // Fetch valid component slugs from daisyui.com
  const validSlugs = await fetchComponentSlugs()
  ...
  for (const dir of componentDirs) {
    ...
    const slug = findSlug(componentName, validSlugs)
    ...
    let docsData = { description: '', cssClasses: [] as string[], examples: [] as CodeExample[] }
    if (slug) {
      docsData = await scrapeDocsPage(slug)
    } else {
      log(`  No matching docs page found for ${componentName}, skipping scrape`)
    }

    const fallbackSlug = componentName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()
    ...
    const component: DaisyUIComponent = {
      ...
      docsUrl: slug ? `https://daisyui.com/components/${slug}/` : '',
...
`
  ...
  // Write .daisyui-version with the current daisyui npm package version
  const rootDir = path.join(__dirname, '..')
  try {
    const daisyuiVersion = execSync('npm view daisyui version', { encoding: 'utf-8' }).trim()
    const versionFilePath = path.join(rootDir, '.daisyui-version')
    fs.writeFileSync(versionFilePath, daisyuiVersion + '\n')
    log(`Wrote daisyUI version ${daisyuiVersion} to .daisyui-version`)
  } catch (error) {
    log(`Warning: could not fetch daisyui version from npm: ${error}`)
  }
